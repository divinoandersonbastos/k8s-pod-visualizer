...
const server = http.createServer(async (req, res) => {
  ...
  // ── /api/network-flows — Mapa de fluxos de rede via eBPF ─────────────────────────────────────────
  if (url.pathname === "/api/network-flows" && req.method === "GET") {
    ...
    try {
      ...
      // Descobrir pods dos agentes eBPF
      const agentPodsResult = await k8sRequest(
        `/api/v1/namespaces/k8s-pod-visualizer/pods?labelSelector=app%3Debpf-agent`
      );
      const agentPods = agentPodsResult?.body?.items || [];

      let rawFlows = [];

      if (agentPods.length > 0 && mode !== "k8s") {
        // Modo eBPF: coletar flows de cada agente em paralelo
        const agentResults = await Promise.allSettled(
          agentPods.map(async (pod) => {
            ...
            try {
              const resp = await fetch(`http://${podIP}:9090/flows${nsQuery}`, {
                signal: AbortSignal.timeout(3000)
              });
              if (!resp.ok) return [];
              const data = await resp.json();
              return (data.flows || []).map(f => ({ ...f, _node: pod.spec?.nodeName }));
            } catch { return []; }
          })
        );
        for (const r of agentResults) {
          if (r.status === "fulfilled") rawFlows.push(...r.value);
        }
      }

      // Fallback: construir fluxos inferidos a partir da API K8s
      if (rawFlows.length === 0 || mode === "k8s") {
        const isSREOrAdmin = ["sre", "admin"].includes(user.role);
        const allowedNs = isSREOrAdmin ? null : (user.namespaces || []);
        const filterNs = (items) => allowedNs
          ? items.filter(i => allowedNs.includes(i.metadata?.namespace))
          : items;

        const nsFilter = ns ? `?fieldSelector=metadata.namespace%3D${ns}` : "";
        const [podsR, svcsR, epsR, ingR] = await Promise.allSettled([
          k8sRequest(`/api/v1/pods${nsFilter}`),
          k8sRequest(`/api/v1/services${nsFilter}`),
          k8sRequest(`/api/v1/endpoints${nsFilter}`),
          k8sRequest(`/apis/networking.k8s.io/v1/ingresses`),
        ]);

        const pods      = filterNs((podsR.status === "fulfilled" ? podsR.value?.body?.items  : null) || []);
        const services  = filterNs((svcsR.status === "fulfilled" ? svcsR.value?.body?.items  : null) || []);
        const eps       = filterNs((epsR.status  === "fulfilled" ? epsR.value?.body?.items   : null) || []);
        const ingresses = filterNs((ingR.status  === "fulfilled" ? ingR.value?.body?.items   : null) || []);

        const ipToPod = {};
        for (const pod of pods) {
          if (pod.status?.podIP) ipToPod[pod.status.podIP] = pod;
        }

        // Fluxos: Internet → Ingress → Service
        for (const ing of ingresses) {
          const ingNs = ing.metadata?.namespace;
          for (const rule of (ing.spec?.rules || [])) {
            for (const p2 of (rule.http?.paths || [])) {
              const svcName = p2.backend?.service?.name;
              if (!svcName) continue;
              rawFlows.push({
                srcIp: "0.0.0.0", dstIp: ing.metadata?.name,
                srcPort: 443, dstPort: 80, protocol: "TCP",
                src: { externalCidr: "internet" },
                dst: { namespace: ingNs, serviceName: ing.metadata?.name },
                bytes: 0, packets: 0,
                firstSeen: new Date().toISOString(), lastSeen: new Date().toISOString(),
                verdict: "forwarded", _inferred: true
              });
              rawFlows.push({
                srcIp: ing.metadata?.name, dstIp: svcName,
                srcPort: 80, dstPort: p2.backend?.service?.port?.number || 80, protocol: "TCP",
                src: { namespace: ingNs, serviceName: ing.metadata?.name },
                dst: { namespace: ingNs, serviceName: svcName },
                bytes: 0, packets: 0,
                firstSeen: new Date().toISOString(), lastSeen: new Date().toISOString(),
                verdict: "forwarded", _inferred: true
              });
        ...
        // Fluxos: Service → Pod (via endpoints)
        for (const ep of eps) {
          ...
          for (const subset of (ep.subsets || [])) {
            for (const addr of (subset.addresses || [])) {
              const pod = ipToPod[addr.ip];
              const podName = pod?.metadata?.name || addr.targetRef?.name || addr.ip;
              const deployName = pod ? (() => {
                const rs = (pod.metadata?.ownerReferences || []).find(r => r.kind === "ReplicaSet");
                if (!rs) return "";
                const parts = rs.name.split("-");
                return parts.length > 1 ? parts.slice(0, -1).join("-") : rs.name;
              })() : "";
              for (const port of (subset.ports || [])) {
                rawFlows.push({
                  srcIp: svcName, dstIp: addr.ip,
                  srcPort: 0, dstPort: port.port, protocol: port.protocol || "TCP",
                  src: { namespace: epNs, serviceName: svcName },
                  dst: { namespace: epNs, podName, deployName },
                  bytes: 0, packets: 0,
                  firstSeen: new Date().toISOString(), lastSeen: new Date().toISOString(),
                  verdict: "forwarded", _inferred: true
                });
      ...
      // Construir grafo de nós e arestas
      const nodeMap = {};
      const edgeMap = {};
      const addNode = (id, type, label, namespace, meta = {}) => {
        if (!nodeMap[id]) nodeMap[id] = { id, type, label, namespace, meta, flowCount: 0, bytes: 0 };
        return nodeMap[id];
      };
      const addEdge = (srcId, dstId, protocol, bytes, packets, verdict, inferred) => {
        const eid = `${srcId}->${dstId}/${protocol}`;
        if (!edgeMap[eid]) edgeMap[eid] = { id: eid, source: srcId, target: dstId, protocol, bytes: 0, packets: 0, verdict, inferred };
        edgeMap[eid].bytes   += bytes   || 0;
        edgeMap[eid].packets += packets || 0;
      };

      for (const flow of rawFlows) {
        const srcNs = flow.src?.namespace || "external";
        const dstNs = flow.dst?.namespace || "external";
        let srcId, dstId;

        if (flow.src?.externalCidr) {
          srcId = `ext:${flow.src.externalCidr}`;
          addNode(srcId, "external",
            flow.src.externalCidr === "internet" ? "Internet" :
            flow.src.externalCidr === "azure" ? "Azure Cloud" : "Rede Interna",
            "external", { cidr: flow.src.externalCidr });
        } else if (flow.src?.serviceName) {
          srcId = `svc:${srcNs}/${flow.src.serviceName}`;
          addNode(srcId, "service", flow.src.serviceName, srcNs);
        } else if (flow.src?.podName) {
          srcId = `pod:${srcNs}/${flow.src.podName}`;
          addNode(srcId, "pod", flow.src.podName, srcNs, { deploy: flow.src.deployName });
        } else {
          srcId = `ip:${flow.srcIp}`;
          addNode(srcId, "ip", flow.srcIp, srcNs);
        }

        if (flow.dst?.externalCidr) {
          dstId = `ext:${flow.dst.externalCidr}`;
          addNode(dstId, "external",
            flow.dst.externalCidr === "internet" ? "Internet" :
            flow.dst.externalCidr === "azure" ? "Azure Cloud" : "Rede Interna",
            "external", { cidr: flow.dst.externalCidr });
        } else if (flow.dst?.serviceName) {
          dstId = `svc:${dstNs}/${flow.dst.serviceName}`;
          addNode(dstId, "service", flow.dst.serviceName, dstNs);
        } else if (flow.dst?.podName) {
          dstId = `pod:${dstNs}/${flow.dst.podName}`;
          addNode(dstId, "pod", flow.dst.podName, dstNs, { deploy: flow.dst.deployName });
        } else {
          dstId = `ip:${flow.dstIp}`;
          addNode(dstId, "ip", flow.dstIp, dstNs);
        }

        if (srcId && dstId && srcId !== dstId) {
          addEdge(srcId, dstId, flow.protocol, flow.bytes, flow.packets, flow.verdict, flow._inferred);
          nodeMap[srcId].flowCount++;
          nodeMap[dstId].flowCount++;
          nodeMap[srcId].bytes += flow.bytes || 0;
          nodeMap[dstId].bytes += flow.bytes || 0;
        }
      }

      const hasEBPF = agentPods.length > 0;
      const flowMode = hasEBPF && mode !== "k8s" ? "ebpf" : "inferred";
      console.log(`[network-flows] mode=${flowMode} flows=${rawFlows.length} nodes=${Object.keys(nodeMap).length} edges=${Object.keys(edgeMap).length} role=${user.role}`);

      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({
        mode: flowMode,
        hasEBPFAgent: hasEBPF,
        agentCount: agentPods.length,
        rawFlowCount: rawFlows.length,
        nodes: Object.values(nodeMap),
        edges: Object.values(edgeMap),
        namespaces: [...new Set(Object.values(nodeMap).map(n => n.namespace).filter(Boolean))],
        timestamp: new Date().toISOString()
      }));
    } catch (err) {
      console.error("[error] /api/network-flows:", err.message);
      res.writeHead(500, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: err.message }));
    }
    return;
